...
To illustrate X.509 authentication, we create a simple scenario with three services. One service will be the backend that is a simple nginx instance serving static data. On the other side, we run two instances of the `Symbank` demo application acting as the frontend services. The `Symbank` demo application simulates a user bank account and sends HTTP requests to the backend to get the user account details. 

![SPIRE Envoy integration diagram][diagram]

[diagram]: images/SPIRE_Envoy_diagram.png "SPIRE Envoy integration diagram"

As shown in the diagram, the frontend services connect to the backend service via a mTLS connection established by the Envoy instances that perform X.509 SVID authentication on workload's behalf.
...
## Update RBAC Rules so only one Frontend Can Access the Backend

Envoy configuration for the `backend` service includes a simple RBAC rule to allow any action from only two authenticated principals, the frontend services.
Let's now update the Envoy configuration for the `backend` service to allow requests from the `frontend` service only.
This is achieved by removing the entry that belongs to the`frontend-2` service from the RBAC rule at the [Envoy configuration](k8s/backend/config/envoy.yaml). The updated rule looks like this:
...
To update the Envoy configuration for the `backend` workload use the file `backend-envoy-configmap-rbac-update.yaml`:
